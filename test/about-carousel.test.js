...
test('About page includes an accessible three-screen app carousel', () => {
  ...
  const overview = fs.readFileSync(path.join(root, 'views', 'partials', 'about-overview.ejs'), 'utf8');
  ...
  assert.match(view, /include\('partials\/about-overview'/);
  assert.match(overview, /aria-roledescription="carousel"/);
  assert.equal((overview.match(/class="app-screenshot"/g) || []).length, 3);
  assert.equal((overview.match(/class="preview-logo"><img src="\/icon\.svg"/g) || []).length, 3);
